function plotmetric_onpatch_both(pth, AZ, FLIPX, FLIPY, metric_file_lh, metric_file_rh, outp)
    ...
    % ---- Left Hemisphere ---- %
    subplot(1, 2, 1);
    plotmetric_onpatch(fullfile(pth, 'patch_lh.mat'), AZ, FLIPX, FLIPY, metric_file_lh, 'Left Hemisphere');
    ...
    % ---- Right Hemisphere ---- %
    subplot(1, 2, 2);
    plotmetric_onpatch(fullfile(pth, 'patch_rh.mat'), AZ, FLIPX, FLIPY, metric_file_rh, 'Right Hemisphere');
...
%     set(cb, 'Position', [0.4 0.05 0.2 0.03]);
    ...
    colormap('jet');
    cb = colorbar('Orientation', 'horizontal');
    clim([0, 0.5]);  % Set the limits of the colorbar
    % Position format [left, bottom, width, height]
    %set(cb, 'Position', [0.92 0.1 0.01 0.8]);  % Positioning the colorbar at the center
    set(cb, 'Position', [0.4 0.05 0.2 0.03]);
...
function plotmetric_onpatch(patch_file, AZ, FLIPX, FLIPY, metric_file, title_text)
...
%     patch('vertices',vtc_roi,'faces',tri_roi,'FaceVertexCData',cmap_roi(cidx_roi,:),'FaceColor','interp','Edgecolor','none','FaceAlpha',0.7);
    ...
    patch('vertices',vtc_roi,'faces',tri_roi,'FaceVertexCData',cmap_roi(cidx_roi,:),'FaceColor','interp','Edgecolor','none','FaceAlpha',0.5);
