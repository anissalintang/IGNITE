function make_patch_fssym(pth,THR,AZ,FLIPX,FLIPY)
%     AZ = 0;
%     THR = 50;
%     FLIPX = 0;
%     FLIPY = 0;
    ...
    surf = struct;
    [~,tri] = read_surf(fullfile(subjs_dir,'fsaverage_sym','surf','lh.white'));
    surf.tri = tri+1;
    surf.vtc.white = read_surf(fullfile(subjs_dir,'fsaverage_sym','surf','lh.smoothwm'));
    surf.vtc.pial = read_surf(fullfile(subjs_dir,'fsaverage_sym','surf','lh.pial'));
    surf.vtc.infl = read_surf(fullfile(subjs_dir,'fsaverage_sym','surf','lh.inflated'));
    surf.vtc.med = (surf.vtc.white+surf.vtc.pial)/2;
    vtc = surf.vtc.med;

    surf.graph = fsurf2graph(vtc,surf.tri); 
    surf.curv = read_curv(fullfile(subjs_dir,'fsaverage_sym','surf','lh.curv')); 

    pth_o = ('/Volumes/gdrive4tb/IGNITE/tonotopy/surface');
    save(fullfile(pth_o,'patch','surf_fssym.mat'))

    % 2. Create patch;  
    mri = MRIread(fullfile(pth,'probActMap','probActMap.lh.fssym.mgz'));
    VOL = mri.vol; 
    mri = MRIread('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/destrieux_fsavg/temporalLobe_mask_lh_fssym.mgh');
    ...
    RAD = max(distances(surf.graph,IDX,idx))*1.3;
    ...
    save(fullfile(pth_o,'patch','patch_fssym.mat'),'ptch','roi') 

    mri.vol = zeros(size(VOL)); mri.vol(ptch.map2surf) = 1;
    MRIwrite(mri,fullfile(fullfile(pth_o,'patch','patch.lh.fssym.mgz')));
    
    mri.vol = zeros(size(VOL)); mri.vol(ptch.map2surf(roi.map2ptch)) = 1;
    MRIwrite(mri,fullfile(fullfile(pth_o,'patch','roi.lh.fssym.mgz')));     
