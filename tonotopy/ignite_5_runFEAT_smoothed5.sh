...
prestats(){
    ...
    globint=$(fslstats $preproc_path/${s}/${s}_preprocessed_smoothed5.nii.gz -k $preproc_path/${s}/meanFunc_smoothed5/bet/${s}_mean_func_smoothed5_bet_mask.nii.gz -p 50)
    fslmaths $preproc_path/${s}/${s}_preprocessed_smoothed5.nii.gz -mul 10000 -div $globint $preproc_path/${s}/${s}_preprocessed_smoothed5_globint.nii.gz

    line=($(fslhd $preproc_path/${s}/${s}_preprocessed_smoothed5_globint.nii.gz | grep pixdim4)); tr=${line[1]}; tf=$(bc -l <<< "100/($tr*2)")


    fslmaths $preproc_path/${s}/${s}_preprocessed_smoothed5_globint.nii.gz -Tmean $preproc_path/$s/${s}_preprocessed_smoothed5_globint_temp.nii.gz
    fslmaths $preproc_path/${s}/${s}_preprocessed_smoothed5_globint.nii.gz -bptf $tf -1 -add $preproc_path/${s}/${s}_preprocessed_smoothed5_globint_temp.nii.gz $preproc_path/${s}/${s}_preprocessed_smoothed5_globint_prestats.nii.gz

    imrm $preproc_path/${s}/${s}_preprocessed_smoothed5_globint_temp.nii.gz
}
export -f prestats
...
runglm(){
    ...
    for design in ${designs[@]}; do
        ...
        if [ ! -d $glm_path/${s}/feat/$featdir ]; then
            ...
            imcp "$preproc_path/${s}/${s}_preprocessed_smoothed5_globint_prestats.nii.gz" "$glm_path/${s}/feat/$featdir/filtered_func_data"
