function make_patch(pth,opt,THR,AZ,FLIPX,FLIPY)
%     pth = fullfile(filesep,'Volumes','gdrive','mri','ProcData','SimHL');
%     opt = 'standard/nordic';
%     THR = 50;
%     AZ = 0;
%     FLIPX = 0;
%     FLIPY = 0;
    ...
    save(fullfile(pth,'patch','surf.mat'),'surf')

    % 2. Create patch;  
    mri = MRIread(fullfile(pth,'surf','actprob.lh.fssym.mgz'));
    VOL = mri.vol; 
    mri = MRIread(fullfile(subjs_dir,'tlbmask.lh.fssym.mgz'));
    ...
    save(fullfile(pth,'patch','patch.mat'),'ptch','roi')   

    mri.vol = zeros(size(VOL)); mri.vol(ptch.map2surf) = 1;
    MRIwrite(mri,fullfile(fullfile(pth,'surf','patch.lh.fssym.mgz')));
    
    mri.vol = zeros(size(VOL)); mri.vol(ptch.map2surf(roi.map2ptch)) = 1;
    MRIwrite(mri,fullfile(fullfile(pth,'surf','roi.lh.fssym.mgz')));     
end
