pth = "/Volumes/gdrive4tb/IGNITE/code/tonotopy/KK";
    ...
    mkdir(fullfile(pth,opt,'glm',SUBJ,'logFiles'))
...
%             nerb{1} = arrayfun(@(x) mean(nerb{2}((x-1)*4+1:(x-1)*4+4)),1:NFrq/4);
    ...
    save(fullfile(pth,opt,'glm',SUBJ,'logFiles','sequence.mat'),'TA','StimDur','NRep','NSil','nfrq','nerb','seq','ev')
    ...
    mkdir(fullfile(pth,opt,'glm',SUBJ,'logFiles',sprintf('all_cmb_%d',nfrq(1))))
    for I = 1:nfrq(1)
