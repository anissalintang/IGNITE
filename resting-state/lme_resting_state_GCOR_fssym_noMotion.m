...
predictorsData = [dataTable.Age, dataTable.PTA_mean, dataTable.AgeSq, Sex_dummy, Hemisphere_dummy, TinnitusStatus_dummy];
...
ax.XTick = 1:6;
ax.YTick = 1:6;
ax.XTickLabel = {'Age', 'PTA_mean', 'AgeSq', 'Sex', 'Hemisphere', 'TinnitusStatus'};
ax.YTickLabel = {'Age', 'PTA_mean', 'AgeSq', 'Sex', 'Hemisphere', 'TinnitusStatus'};
...
filename = '/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/without_motionParam/GCOR_lme_noMotion.mat';
...
for idx = 1:length(alphas)
    ...
    MRIwrite(imgDataUncorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/without_motionParam/fullmod/GCOR_tmap_tinStat_fullMod_uncorrected_%g.mgz', alpha));
    ...
    MRIwrite(imgDataCorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/without_motionParam/fullmod/GCOR_tmap_tinStat_fullMod_corrected_%g.mgz', alpha));
end
...
for idx = 1:length(alphas)
    ...
    MRIwrite(imgDataUncorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/without_motionParam/fullmod/GCOR_tmap_PTA_mean_fullMod_uncorrected_%g.mgz', alpha));
    ...
    MRIwrite(imgDataCorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/without_motionParam/fullmod/GCOR_tmap_PTA_mean_fullMod_corrected_%g.mgz', alpha));
end
...
for idx = 1:length(alphas)
    ...
    MRIwrite(imgDataUncorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/without_motionParam/fullmod/GCOR_tmap_hemis-PTA_fullMod_uncorrected_%g.mgz', alpha));
    ...
    MRIwrite(imgDataCorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/without_motionParam/fullmod/GCOR_tmap_hemis-PTA_fullMod_corrected_%g.mgz', alpha));
end
...
for idx = 1:length(alphas)
    ...
    MRIwrite(imgDataUncorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/without_motionParam/fullmod/GCOR_tmap_Age_fullMod_uncorrected_%g.mgz', alpha));
    ...
    MRIwrite(imgDataCorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/without_motionParam/fullmod/GCOR_tmap_Age_fullMod_corrected_%g.mgz', alpha));
end
...
% Find the column corresponding to PTA_mean
AgeSqColumn = find(strcmp(predictorNames, 'AgeSq'));
...
for idx = 1:length(alphas)
    ...
    % Uncorrected
    significantVerticesUncorrected = pValues(:, AgeSqColumn) < alpha;
    AgeSqTValuesSignificantUncorrected = coefficients(significantVerticesUncorrected, AgeSqColumn);

    imgDataUncorrected = MRIread(files{file});
    imgDataUncorrected.vol(:) = 0;
    imgDataUncorrected.vol(significantVerticesUncorrected) = AgeSqTValuesSignificantUncorrected;
    MRIwrite(imgDataUncorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/without_motionParam/fullmod/GCOR_tmap_AgeSq_fullMod_uncorrected_%g.mgz', alpha));
    
    % Corrected
    significantVerticesCorrected = fdrCorrectedPValues(:, AgeSqColumn) < alpha;
    AgeSqTValuesSignificantCorrected = coefficients(significantVerticesCorrected, AgeSqColumn);

    imgDataCorrected = MRIread(files{file});
    imgDataCorrected.vol(:) = 0;
    imgDataCorrected.vol(significantVerticesCorrected) = AgeSqTValuesSignificantCorrected;
    MRIwrite(imgDataCorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/without_motionParam/fullmod/GCOR_tmap_AgeSq_fullMod_corrected_%g.mgz', alpha));
end
...
%% Fitlme with parallel with only age, ageSq, sex, tinnitusStatus, PTA_mean
...
% Initialize matrices to hold the coefficients and p-values
numCoefficients = size(fitlme(dataTable(dataTable.VertexID == uniqueVertices(1), :), 'Response ~ 1 + Sex + Age + PTA_mean + AgeSq + TinnitusStatus + (1|SubjectID)').Coefficients, 1);
...
for i = 1:length(uniqueVertices)
    ...
    % Define the formula for the LME model
    lmeFormula_mod2 = 'Response ~ 1 + Sex + Age + PTA_mean + AgeSq + TinnitusStatus + (1|SubjectID)';
...
%% P values uncorrected
lmeFormula_mod2 = 'Response ~ 1 + Age + AgeSq + Sex + PTA_mean + TinnitusStatus + (1|SubjectID)';
% Set alpha values to the specified range
...
for idx = 1:length(alphas)
    ...
    MRIwrite(imgDataUncorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/mod2/GCOR_tmap_tinStat_mod2_uncorrected_%g.mgz', alpha));
    ...
    MRIwrite(imgDataCorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/mod2/GCOR_tmap_tinStat_mod2_corrected_%g.mgz', alpha));
end
...
for idx = 1:length(alphas)
    ...
    MRIwrite(imgDataUncorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/mod2/GCOR_tmap_PTA_mean_mod2_uncorrected_%g.mgz', alpha));
    ...
    MRIwrite(imgDataCorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/mod2/GCOR_tmap_PTA_mean_mod2_corrected_%g.mgz', alpha));
end
...
for idx = 1:length(alphas)
    ...
    MRIwrite(imgDataUncorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/mod2/GCOR_tmap_Age_mod2_uncorrected_%g.mgz', alpha));
    ...
    MRIwrite(imgDataCorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/mod2/GCOR_tmap_Age_mod2_corrected_%g.mgz', alpha));
end
...
% Find the column corresponding to AgeSq
AgeSqColumn_mod2 = find(strcmp(predictorNames_mod2, 'AgeSq'));
...
for idx = 1:length(alphas)
    ...
    % Uncorrected
    significantVerticesUncorrected_mod2 = pValues_mod2(:, AgeSqColumn_mod2) < alpha;
    AgeSqTValuesSignificantUncorrected_mod2 = coefficients_mod2(significantVerticesUncorrected_mod2, AgeSqColumn_mod2);

    imgDataUncorrected = MRIread(files{file});
    imgDataUncorrected.vol(:) = 0;
    imgDataUncorrected.vol(significantVerticesUncorrected_mod2) = AgeSqTValuesSignificantUncorrected_mod2;
    MRIwrite(imgDataUncorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/mod2/GCOR_tmap_AgeSq_mod2_uncorrected_%g.mgz', alpha));
    
    % Corrected
    significantVerticesCorrected_mod2 = fdrCorrectedPValues_mod2(:, AgeSqColumn_mod2) < alpha;
    AgeSqTValuesSignificantCorrected_mod2 = coefficients_mod2(significantVerticesCorrected_mod2, AgeSqColumn_mod2);

    imgDataCorrected = MRIread(files{file});
    imgDataCorrected.vol(:) = 0;
    imgDataCorrected.vol(significantVerticesCorrected_mod2) = AgeSqTValuesSignificantCorrected_mod2;
    MRIwrite(imgDataCorrected, sprintf('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/mod2/GCOR_tmap_AgeSq_mod2_corrected_%g.mgz', alpha));
end
...
%% Save all analysis to mat files
% Define the filename for the .mat file
filename = '/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/GCOR_fssym/stat/GCOR_lme.mat';
