...
subjects = subjects(~ismember({subjects.name}, {'.', '..','cortex_mask', ...
    'hearingEffect', 'motionEffect', 'ageEffect','merge_mean','tinEffect', ...
    '.DS_Store','meanValues','meanValues_cortex'}));
...
diff_lh = mean(cat(1, older_lh.imgData), 1) - mean(cat(1, younger_lh.imgData), 1);
diff_rh = mean(cat(1, older_rh.imgData), 1) - mean(cat(1, younger_rh.imgData), 1);
...
MRIwrite(diff_lh_img, '/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/ALFF/ageEffect/ALFF_lh_ageEffect.mgz');
MRIwrite(diff_rh_img, '/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/ALFF/ageEffect/ALFF_rh_ageEffect.mgz');
...
diffPercentile_lh = mean(cat(1, olderPercentile_lh.imgData), 1) - mean(cat(1, youngerPercentile_lh.imgData), 1);
diffPercentile_rh = mean(cat(1, olderPercentile_rh.imgData), 1) - mean(cat(1, youngerPercentile_rh.imgData), 1);
...
MRIwrite(diffPercentile_lh_img, '/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/ALFF/ageEffect/ALFF_lh_ageEffect_percentiles.mgz');
MRIwrite(diffPercentile_rh_img, '/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/ALFF/ageEffect/ALFF_rh_ageEffect_percentiles.mgz');
