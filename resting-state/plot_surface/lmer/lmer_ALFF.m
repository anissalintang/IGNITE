...
%% ALFF tmap PTA - Uncorrected -with_motionParam

% List of images
images = {'ALFF_tmap_PTA_mean_fullMod_uncorrected_0.01.mgz', 
          'ALFF_tmap_PTA_mean_fullMod_uncorrected_0.05.mgz',
          'ALFF_tmap_PTA_mean_fullMod_uncorrected_0.1.mgz',
          'ALFF_tmap_PTA_mean_fullMod_uncorrected_0.25.mgz',
          'ALFF_tmap_PTA_mean_fullMod_uncorrected_0.5.mgz'};
...
for i = 1:length(images)
    ...
    cmap_roi = colormap(cmap_custom);
%     scaled_data = (min(max(data, (max(data)*-1)), max(data)) + max(data)) / (max(data)*2);
    scaled_data = (min(max(data, -0.05), 0.05) + 0.05) / (0.05*2);
    cidx_roi = round(scaled_data * (size(cmap_roi,1)-1)) + 1;
    ...
    value = value{end};
    value = value(1:end-4);
    sgtitle(['ALFF t-values map of PTA. Uncorrected (' value ')'])

    % Save the figure
    saveas_name = fullfile('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/keyFigures/lmer_tmaps/ALFF/with_motionParam/', ['ALFF_tmap_PTA_mean_uncorrected_' value '.png']);
    saveas(gcf, saveas_name);
...
% List of images
images = {'ALFF_tmap_PTA_mean_fullMod_corrected_0.01.mgz', 
          'ALFF_tmap_PTA_mean_fullMod_corrected_0.05.mgz',
          'ALFF_tmap_PTA_mean_fullMod_corrected_0.1.mgz',
          'ALFF_tmap_PTA_mean_fullMod_corrected_0.25.mgz',
          'ALFF_tmap_PTA_mean_fullMod_corrected_0.5.mgz'};
...
for i = 1:length(images)
    ...
    value = value{end};
    value = value(1:end-4);
    sgtitle(['ALFF t-values map of PTA. FDR corrected (' value ')'])

    % Save the figure
    saveas_name = fullfile('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/keyFigures/lmer_tmaps/ALFF/with_motionParam/', ['ALFF_tmap_PTA_mean_corrected_' value '.png']);
    saveas(gcf, saveas_name);
