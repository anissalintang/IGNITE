%% recov function to calculate temporal mask

function calc_recov_temporal_condition(varargin)

    if nargin<=3
        error('Not enough input variables')
    else
        data = varargin{1};
        recov_lh_map_out_rest = varargin{2};
        recov_rh_map_out_rest = varargin{3};
        recov_lh_map_out_vis = varargin{4};
        recov_rh_map_out_vis = varargin{5};
        ...
        % Calculate recov for every vertex in the mask separately for each condition and hemisphere
        recov_lh_rest = calc_recov_for_condition(U_lh_rest, U_lh_mask_rest);
        recov_rh_rest = calc_recov_for_condition(U_rh_rest, U_rh_mask_rest);
        recov_lh_vis = calc_recov_for_condition(U_lh_vis, U_lh_mask_vis);
        recov_rh_vis = calc_recov_for_condition(U_rh_vis, U_rh_mask_vis);

        % Save recov data for rest condition
        save_recov_data(recov_lh_rest, recov_lh_map_out_rest, 'lh');
        save_recov_data(recov_rh_rest, recov_rh_map_out_rest, 'rh');

        % Save recov data for vis condition
        save_recov_data(recov_lh_vis, recov_lh_map_out_vis, 'lh');
        save_recov_data(recov_rh_vis, recov_rh_map_out_vis, 'rh');

    end
end

function recov_condition = calc_recov_for_condition(U_condition, U_mask_condition)
    recov_condition = [];
    g = mean(U_mask_condition, 2, 'omitnan');
    N = size(g, 1);
    for i = 1:size(U_condition, 2)
        recov_condition = [recov_condition, 1 / N * U_condition(:, i)' * g];
    end
end
