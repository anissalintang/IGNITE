...
mkdir -p "/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/ReHo/temporal/merge_mean"
...
# 						tin_rest+="'${reho_path}/${s}/${s}_ReHo_${h}_temporal_rest.mgz',"
						
# 					elif [[ $f == *"${h}_temporal_vis.mgz" ]]; then
# 						tin_vis+="'${reho_path}/${s}/${s}_ReHo_${h}_temporal_vis.mgz',"
...
# 				for f in ${files[@]}; do
# 					if [[ $f == *"${h}_temporal_rest.mgz" ]]; then
# 						noTin_rest+="'${reho_path}/${s}/${s}_ReHo_${h}_temporal_rest.mgz',"
						
# 					elif [[ $f == *"${h}_temporal_vis.mgz" ]]; then
# 						noTin_vis+="'${reho_path}/${s}/${s}_ReHo_${h}_temporal_vis.mgz',"
...
for s in ${subj[@]}; do
	for h in ${hemi[@]}; do
		for c in ${cond[@]}; do
			mri_surf2surf \
			--sval ${reho_path}/${s}/${s}_ReHo_${h}_temporal_${c}.mgz \
			--tval ${reho_path}/${s}/${s}_ReHo_${h}_temporal_${c}_smooth5.mgz \
			...
			if [[ $s == *"TT"* ]]; then
				...
				for f in ${files[@]}; do
					if [[ $f == *"${h}_temporal_rest_smooth5.mgz" ]]; then
						tin_rest+="'${reho_path}/${s}/${s}_ReHo_${h}_temporal_rest_smooth5.mgz',"
						
					elif [[ $f == *"${h}_temporal_vis_smooth5.mgz" ]]; then
						tin_vis+="'${reho_path}/${s}/${s}_ReHo_${h}_temporal_vis_smooth5.mgz',"
				...
				for f in ${files[@]}; do
					if [[ $f == *"${h}_temporal_rest_smooth5.mgz" ]]; then
						noTin_rest+="'${reho_path}/${s}/${s}_ReHo_${h}_temporal_rest_smooth5.mgz',"
						
					elif [[ $f == *"${h}_temporal_vis_smooth5.mgz" ]]; then
						noTin_vis+="'${reho_path}/${s}/${s}_ReHo_${h}_temporal_vis_smooth5.mgz',"
