%% Calculate ALFF from sparse rest scans
clear
clc
...
for i = 1:length(sub_dirs)
    sub_dir = sub_dirs{i};
    mkdir(fullfile('/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/'), sub_dir);

        save_path_lh = fullfile('/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/', sub_dir, '/', [sprintf(sub_dir), '_lh_', output_names{1}, '.mgz']);
        save_path_rh = fullfile('/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/', sub_dir, '/',  [sprintf(sub_dir), '_rh_', output_names{1}, '.mgz']);
...
for i = 1:length(sub_dirs)
    sub_dir = sub_dirs{i};
        save_path_lh = fullfile('/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/', sub_dir, '/', [sprintf(sub_dir), '_lh_', output_names{2}, '.mgz']);
        save_path_rh = fullfile('/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/', sub_dir, '/', [sprintf(sub_dir), '_rh_', output_names{2}, '.mgz']);
