...
%% ALFFall noTin
img1 = MRIread('/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/ALFFall/merge_mean/allSubj_lh_ALFFall_merged_mean_noTin_smooth5.mgz');
img2 = MRIread('/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/ALFFall/merge_mean/allSubj_rh_ALFFall_merged_mean_noTin_smooth5.mgz');
data1 = img1.vol';
data2 = img2.vol';
...
scaled_data = (min(max(all_data, -1.5), 1.5) + 1.5) / (1.5*2);
cidx_roi = round(scaled_data * (size(cmap_roi,1)-1)) + 1;

% Set transparency for data less than 0
alpha_lhs = ones(size(data1));
alpha_lhs(data1 == 0) = 0; % Set alpha to 0 for data < 0
patch('Faces',lh_faces+1,'Vertices',lh_vertices,'FaceVertexCData',cmap_roi(cidx_roi(1:numel(data1)),:),'FaceColor','interp','EdgeColor','none','FaceAlpha','interp','FaceVertexAlphaData',alpha_lhs,'AlphaDataMapping','none');
...
% Map data to a color
% Set transparency for data less than 0
alpha_rhs = ones(size(data2));
alpha_rhs(data2 == 0) = 0; % Set alpha to 0 for data < 0
patch('Faces',rh_faces+1,'Vertices',rh_vertices,'FaceVertexCData',cmap_roi(cidx_roi(numel(data1)+1:end),:),'FaceColor','interp','EdgeColor','none','FaceAlpha','interp','FaceVertexAlphaData',alpha_rhs,'AlphaDataMapping','none');
...
saveas(gcf, '/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/keyFigures/allSubj_ALFF_merge_mean_noTin_smooth5.png');


%% ALFFall tin
img1 = MRIread('/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/ALFFall/merge_mean/allSubj_lh_ALFFall_merged_mean_tin_smooth5.mgz');
img2 = MRIread('/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/ALFFall/merge_mean/allSubj_rh_ALFFall_merged_mean_tin_smooth5.mgz');
data1 = img1.vol';
data2 = img2.vol';
...
saveas(gcf, '/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/keyFigures/allSubj_ALFF_merge_mean_tin_smooth5.png');
