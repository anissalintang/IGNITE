function make_patch_lh_man(pth,AZ,FLIPX,FLIPY)
%     AZ = 0;
%     FLIPX = 0;
%     FLIPY = 0;
    ...
    save(fullfile(pth_o,'patch','surf_lh.mat'),'surf')

    % 2. Create patch;  
    mri = MRIread('/Volumes/gdrive4tb/IGNITE/resting-state/surface/analysis/colin27_fsavg/HO_HG_lh_mask_fsavg.mgz');
    idx = find(mri.vol>0);
...
%     [~, IDX] = min(dist_sq)
    ...
    %
    % eg below my index is 685 for angulation parameter of 160,0,0
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Do it manually now for lh using (160,0,0) IDX is 685
    IDX = 685;
    ...
    % Calculate RAD, max distance * 1
    RAD = max(distances(subgrph, IDX))*1;
    ...
    save(fullfile(pth_o,'patch','patch_lh.mat'),'ptch','roi')   
