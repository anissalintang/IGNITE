...
subjects = subjects(~ismember({subjects.name}, {'.', '..','merge_mean', ...
    'tinEffect','ageEffect','stimEffect','stimEffect_tin', '.DS_Store', 'meanValues'})); % Removed other unwanted directories
...
ageEffect_lh = mean(cat(1, older_lh.imgData), 1) - mean(cat(1, younger_lh.imgData), 1);
ageEffect_rh = mean(cat(1, older_rh.imgData), 1) - mean(cat(1, younger_rh.imgData), 1);

% Save the age effect as images in .mgz
saveAgeEffect(ageEffect_lh, imgDataOri, '/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/ALFF/ageEffect/ALFF_lh_ageEffect_pctl.mgz');
saveAgeEffect(ageEffect_rh, imgDataOri, '/Volumes/gdrive4tb/IGNITE/sparse_rest/surface/analysis/ALFF/ageEffect/ALFF_rh_ageEffect_pctl.mgz');
